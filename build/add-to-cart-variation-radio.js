...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var matchVariation = function matchVariation(attributes, variation) {
    try {
      attributes.forEach(function (data) {
        if (data.attributes[variation.name] && data.attributes[variation.name] === variation.value) {
          throw "true";
        }
  ...
  var radioGetChosenAttributes = function radioGetChosenAttributes(form) {
    ...
    var count = 0;
    var chosen = 0;
    form.querySelectorAll(".variations .wrb-list").forEach(function (radioList) {
      count++;
      /*
      ...
      if (selected.parentElement.classList.contains("radio__none")) return false;
  ...
  var disableButton = function disableButton(button) {
    ...
    button.classList.remove("active"); // console.log(button.name + button.value + " disabled");
  };
  /**
  ...
  var enableButton = function enableButton(button) {
    button.classList.remove("disabled"); // console.log(button.name + ":" + button.value + " enabled");
  };
  ...
  document.querySelectorAll(".variations_form").forEach(function (form) {
    ...
    inputs.forEach(function (current) {
      ...
      current.addEventListener("click", function () {
        ...
        if (!form.dataset.length) $form.trigger("woocommerce_variation_select_change");
        /*
        ...
        if (current.classList.contains("active")) {
          ...
          parentUL.querySelectorAll("input").forEach(function (inputWithTerm) {
            inputWithTerm.classList.remove("active");
          });
          var radioNone = parentUL.querySelector(".radio__none input");
          radioNone.click();
        } else {
          /*
           ...
           */
          var chosenAttributes = radioGetChosenAttributes(form);
          /*
           * Fetch the variations array in order to find matching terms
           */

          var matchingVariations = chosenAttributes ? findMatchingVariations(formData, chosenAttributes.data) : {}; // then for each input
          ...
          inputs.forEach(function (input) {
            ...
            if (input.name === current.name || !current.value) {
              if (!current.value) {
                ...
                enableButton(input);
                if (input.value && !input.checked) input.classList.remove("active");
            ...
            } else if (input.checked) {// enableButton(input);
            } else if ( // if the button is the none button or
